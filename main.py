...
def DEM_loading(dict_algorithm, dict_geometry, dict_material, dict_sample, dict_sollicitation, simulation_report):
    ...
    while DEM_loop_statut :
        ...
        #Move grains
        for grain in dict_sample['L_g']:
            grain.euler_semi_implicite(dict_algorithm['dt_DEM'], dict_algorithm['factor_neighborhood']*grain.radius/dict_algorithm['dt_DEM']/dict_algorithm['i_update_neighborhoods'])
        ...
        mean_n_contact_tracker.append((2*len(dict_sample['L_contact'])+len(dict_sample['L_contact_gw']))/len(dict_sample['L_g']))
